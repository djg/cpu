...
pub fn set_rand_reset(val: RandomMode) {
    ...
    unsafe { ffi::verilated_set_rand_reset(val); }
}
...
pub fn set_finish() {
    unsafe { ffi::verilated_set_got_finish(1); }
}
...
pub fn command_args(args: Vec<CString>) {
    ...
    unsafe { ffi::verilated_command_args(args.len() as c_int,
...
pub fn internals_dump() {
    unsafe { ffi::verilated_internals_dump(); }
}
...
/// releases - contact the authors before production use.
pub fn scopes_dump() {
    unsafe { ffi::verilated_scopes_dump(); }
}
